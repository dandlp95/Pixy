...
exports.editUser = async (req, res) => {
  try {
    ...
    if (result) {
      res.status(200).json(result[0]);
      console.log(
        `Modified User: ${req.params.id}! From the userController file.`
      );
    } else {
      res
        .status(500)
        .json(response.error || "An error occurred while modifing the user.");
    }
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    if (result) {
      res.status(200).json(result[0]);
      console.log(
        `Deleted User: ${req.params.id}! From the userController file.`
      );
    } else {
      res
        .status(500)
